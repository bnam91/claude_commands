...
### 1. 목록 이름만 빠르게 확인
```
python ~/Documents/github_skills/app_reminder_control/app_reminders_control.py --lists
```
...
```bash
python3 -c "
import sys; sys.path.insert(0, '~/Documents/github_skills/app_reminder_control')
...
### 3. 특정 목록의 미리알림 조회
```bash
python ~/Documents/github_skills/app_reminder_control/app_reminders_control.py [번호 또는 목록이름]
```
예시:
- `python ... 1` → 1번 목록 조회
...
### 4. 미리알림 추가 (섹션 지정)
```bash
python ~/Documents/github_skills/app_reminder_control/app_reminders_control.py [목록번호] "섹션이름, 항목제목"
```
예시:
