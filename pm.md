...
| 키워드 | 프로젝트명 | 스킬 | 설명 |
|--------|-----------|------|------|
| `gg` | GG | `/pm-gg` | 딜롱·사입 3.20 런칭 목표 |
| `cc` | CC | `/pm-cc` | 소싱루틴을 통한 쿠팡 첫 판매 3.31 런칭 |
| `xx` | XX | `/pm-xx` | 역전야매치트키 유튜브 채널, 연매출 2억~2.5억 목표 |
...
PM들의 브리핑은 두 가지 채널로 분리된다.

### 채널 구분

| 채널 | 스킬 | 대상 DB | 용도 |
|------|------|---------|------|
| **Notion meeting DB** | `/agent_meeting`, `/pm_cc_weekly` 등 | `31c111a5778880a68164f8f27f2463c8` | 전체 PM 공통 회의 페이지 |
| **프로젝트 브리핑 DB** | 아래 브리핑 프로세스 | 각 프로젝트 브리핑 DB ID | 일일 시작·마감 브리핑 (GG 전용) |

### Meeting 플로우 (주간)

```
1. /agent_weekly_meet     → 이번주 weekly meeting 페이지 생성 (Notion meeting DB)
2. /pm_cc_weekly          → pm-cc 브리핑 자동 작성 (DB 분석 + 단계 감지)
3. /pm_gg_weekly          → pm-gg 브리핑 작성 (--content로 직접 입력)
4. /pm_xx_weekly          → pm-xx 브리핑 작성 (--content로 직접 입력)
5. rich-agent 종합 피드백  → "Rich Agent 종합 피드백" 토글에 작성 (별도 처리)
```

### Meeting 플로우 (오전)

```
1. /agent_meeting --morning  → 오늘 morning meeting 페이지 생성
2. /pm_cc_morning            → pm-cc 오전 브리핑
3. /pm_gg_morning            → pm-gg 오전 브리핑
4. /pm_xx_morning            → pm-xx 오전 브리핑
```

### 브리핑 규칙 참조

`~/Documents/claude_skills/agent_meeting/briefing_rules.md` — 섹션 구조, 단계별 필터링 기준, 나쁜 예/좋은 예

---

## 프로젝트별 DB 필드명 차이

pm.md의 공통 DB 구조는 GG 기준. CC는 필드명이 다름에 주의.

| 항목 | GG (공통) | CC |
|------|-----------|----|
| 제목 필드 | `TASK` (title) | `업무` (title) |
| 상태 필드 | `상태` (status 타입) | `4_상태` (select 타입) |
| 우선순위 필드 | `우선순위` (status 타입) | `1_우선순위` (select 타입) |
| 상태 쿼리 방식 | `{ "status": { "equals": "진행 중" } }` | `{ "select": { "equals": "진행 중" } }` |
...
- PM 코멘트는 보고서 문체가 아닌 **구어체 존댓말**로 작성합니다
- 📍 설명 문구에도 동일하게 적용합니다
...
트리거: `/pm-interview-cc` / `/pm-interview-gg` / `/pm-interview-xx`
또는: "cc 면담해줘" / "gg 점검하자" / "xx 현황 얘기하자"

현재 프로젝트 DB를 조회한 후 PM 역할로 현빈과 1:1 대화를 진행하고, 면담 중 확정된 내용을 DB에 반영합니다.

#### 면담 진행 흐름

**1단계: DB 조회**

프로젝트 스킬 파일(pm-cc.md 등)의 `프로젝트 DB ID`와 DB 필드명 구조를 참조해서 현황 전체를 조회한다.
- CC: `31c111a5778881a89626ceef93de198b` / 필드: `업무`(title), `4_상태`(select), `1_우선순위`(select)
- GG: `2f6111a5778881ceaf1be4e73f6644ea` / 필드: `TASK`(title), `상태`(status), `우선순위`(status)
- XX: `31c111a5778881dfaa2bd7121a81563d` / 필드: `업무`(title), `4_상태`(select), `1_우선순위`(select)

**2단계: 면담 오프닝 — PM이 먼저 분석해서 가져온다**

조회 결과를 바탕으로 PM이 먼저 아래 4가지를 분석·정리해서 현빈에게 제시한다.
현빈은 듣고 답하는 구조. PM이 준비해서 오는 것이지, 현빈한테 "뭐 해요?" 묻는 게 아니다.

```
안녕하세요. [프로젝트명] 면담 시작하겠습니다.
DB 전체를 보고 왔는데, 먼저 제가 파악한 내용 정리해드릴게요.

① 현재 상황 평가
   "현재 [단계명] 단계입니다. D-XX인데 진행 중 N건, 블로킹 N건입니다.
    [속도에 대한 PM 판단 — 정상 / 위험 / 심각]"

② 애로사항 / 블로킹
   "막혀 있는 항목이 있습니다: [항목명]. [막힌 이유 추정 또는 사실]
    이게 해소되지 않으면 [다음에 어떤 문제가 생기는지] 설명."

③ 현빈한테 필요한 것
   "제가 혼자 처리할 수 없어서 현빈이 직접 결정해줘야 할 것들입니다:
    - [항목명]: [무엇이 필요한지 구체적으로]"

④ DB에서 이해가 안 되는 부분
   "DB를 보다가 잘 모르겠는 항목들이 있어서요:
    - '[항목명]' — 이게 지금 어떤 상태인지, 제가 어떻게 관리하면 될까요?
    - '[항목명]' — 이 업무가 현재 단계에서 맞는 건지 확인이 필요합니다."
...
- 현빈의 답변에서 DB 변경 사항을 감지해 메모해둡니다:

| 현빈 발언 | 감지 | DB 변경 |
|-----------|------|---------|
| "완료했어" / "끝났어" / "다했어" | 완료 | `4_상태` → `완료` |
| "하고 있어" / "진행 중" | 진행 중 | `4_상태` → `진행 중` |
| "막혔어" / "못 하고 있어" | 블로킹 | `4_상태` → `업무막힘` |
| "나중에" / "아직" | 대기 유지 | 변경 없음 |
| "우선순위 바꿔줘" | 우선순위 | `1_우선순위` 수정 |
| "빼줘" / "없애줘" | 삭제 확인 후 처리 | |

- 현빈이 설명하면 PM은 **그게 DB에 어떻게 반영되어야 하는지 제안**하고 확인을 구합니다
  예: "그러면 이 항목은 '진행 중'으로 바꾸면 될까요?"
- 면담 중 변경사항은 **즉시 반영하지 않고** 메모 후 면담 종료 시 일괄 처리합니다

**4단계: 면담 결과 — personal interview DB에 페이지 생성**

면담 시작 직후, 아래 구조로 `personal interview` DB(`31d111a57788807eb6c6e328ea28091d`)에 페이지를 생성합니다.
페이지 제목: `[pm명] 면담 — YYYY-MM-DD`

```
📋 [pm명] | [상품/프로젝트명] | D-XX (기준일) | 면담일: YYYY-MM-DD
...
면담 종료 시 "💬 면담 결론" 섹션을 업데이트(`PATCH /blocks/{id}`)하고 DB 변경사항을 반영합니다.
...
확인 후 Notion API `PATCH /pages/{id}`로 일괄 업데이트합니다.
...
curl -s -X PATCH "https://api.notion.com/v1/pages/{PAGE_ID}" \
  ...
  -d '{ "properties": { "4_상태": { "select": { "name": "완료" } } } }'
```
...
트리거: "프로젝트 만들어줘" / "새 프로젝트 추가" / "프로젝트 셋업해줘"

**진행 순서**

1. 아래 항목 확인 (없으면 사용자에게 질문):
   - `--keyword` : 프로젝트 키워드 (영문 소문자, 예: `cc`)
   - `--desc` : 프로젝트 설명 (없으면 `"입력 예정"` 사용)
   - `--members` : 담당 인원 2명 (예: `"현빈,수지"`)
   - `--goal` : 프로젝트 목표 (예: `"4.30 런칭"`)
   - `--dday` : D-day 기준일 `YYYY-MM-DD` 형식
   - `--levels` : C-레벨 목록 (생략 시 기본값 `CEO,COO,CMO,CFO,CTO` 사용)

2. 아래 명령 실행:

```bash
cd ~/Documents/claude_skills/notion && node create_project.js \
  --keyword <키워드> \
  --desc "<프로젝트 설명>" \
  --members "<이름1,이름2>" \
  --goal "<목표>" \
  --dday "<YYYY-MM-DD>" \
  [--levels "CEO,COO,CMO,CFO,CTO"]
```
...
| 항목 | 내용 |
|------|------|
| Notion 프로젝트 페이지 | `project-manager` 하위에 `project_manager_{keyword}` 페이지 생성 (아이콘 🏇) |
| 페이지 구성 | 설명 콜아웃 → PM 브리핑 DB → 담당자별 업무요청 콜아웃 → 애자일 테이블 DB |
| PM 브리핑 DB | 빈 인라인 DB 생성 (`이름` 컬럼만, 매일 페이지 추가해서 사용) |
| 업무요청 콜아웃 | 담당자별 H2 + 콜아웃 블록 생성, ID 출력 |
| 애자일 테이블 DB | 공통 DB 구조로 생성, C-레벨 행 포함 |
| `pm-{keyword}.md` | `~/.claude/commands/`에 자동 생성 (모든 ID 포함) |
| `pm.md` | 프로젝트 목록 테이블에 자동 추가 |
...
## 공통 DB 구조 (모든 프로젝트 공통) = 애자일 테이블 DB
