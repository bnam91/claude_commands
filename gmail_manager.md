...
### 계정 목록 보기
```bash
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-accounts
```

### 계정 추가
```bash
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --add-account --alias 별칭 --email 이메일주소
```
처음 추가 시 브라우저 OAuth 로그인 필요. 이후는 자동 인증.

### 계정 제거
```bash
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --remove-account --alias 별칭
```
...
```bash
# 기본 (받은편지함 20개)
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-mails --account 별칭

# 한국 시간 기준 오늘 메일
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-mails --account 별칭 --today

# 오늘 보낸 메일
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-mails --account 별칭 --today --query "in:sent"

# 검색 쿼리 포함
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-mails --account 별칭 --query "is:unread"

# 개수 지정
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --list-mails --account 별칭 --max 50
```

### 메일 읽기
```bash
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --read-mail --account 별칭 --id 메일ID
```
...
3. **계정 추가**
```bash
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --add-account --alias work --email your@gmail.com
```

### 휴지통 이동
```bash
# 단일
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --trash-mail --account 별칭 --id 메일ID

# 여러 개
cd ~/Documents/claude_skills/gmail_manager && python3 gmail_manager.py --trash-mail --account 별칭 --ids ID1,ID2,ID3
```
