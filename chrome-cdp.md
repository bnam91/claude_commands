...
| 인수 | 계정 | user-data-dir | 소스 (최초 복사 원본) |
|------|------|---------------|----------------------|
| (없음) | 격리/임시 | `/tmp/chrome-debug` | — |
| `-coq3820` | coq3820@gmail.com (현빈개인) | `~/Documents/github_cloud/user_data/coq3820` | `/Users/a1/Library/Application Support/Google/Chrome/Default` |
| `-bnam91` | bnam91@goyamkt.com (고야) | `~/Documents/github_cloud/user_data/coupangWing_bnam91` | — |
...
case "$ALIAS" in
  coq3820)
    USER_DATA_DIR="~/Documents/github_cloud/user_data/coq3820"
    SOURCE_DIR="~/Library/Application Support/Google/Chrome/Default"
    ;;
  bnam91)
...
# 프로필 디렉토리 없으면 소스에서 복사
if [ -n "$SOURCE_DIR" ] && [ ! -d "$USER_DATA_DIR" ]; then
  echo "🔄 프로필 복사 중: $SOURCE_DIR → $USER_DATA_DIR"
  mkdir -p "$USER_DATA_DIR"
  cp -r "$SOURCE_DIR/." "$USER_DATA_DIR/Default/"
fi
```
...
```bash
# 현재 CDP Chrome의 user-data-dir 확인
CURRENT_PROFILE=$(ps aux | grep -o "remote-debugging-port=9222" | head -1 | xargs -I{} ps aux | grep "remote-debugging-port=9222" | grep -o "user-data-dir=[^ ]*" | cut -d= -f2)

# 또는 더 정확하게:
CURRENT_PROFILE=$(ps aux | grep "remote-debugging-port=9222" | grep -v grep | grep -o "\-\-user-data-dir=[^ ]*" | cut -d= -f2)

TARGET_DIR=$(eval echo "$USER_DATA_DIR")  # ~ 확장
...
인수로 받은 `USER_DATA_DIR` 사용 (없으면 `/tmp/chrome-debug`):
...
- `coq3820` 프로필은 최초 실행 시 실제 Chrome Default에서 `~/Documents/github_cloud/user_data/coq3820`로 자동 복사
- 복사 후에는 독립 프로필이므로 실제 Chrome과 쿠키가 분리됨 (세션 만료 시 재복사 필요)
- 재복사 원할 때: `rm -rf ~/.chrome-profiles/coq3820` 후 재실행
